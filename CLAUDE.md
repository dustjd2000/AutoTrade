...
- 1호 전략(`src/strategy/llm_momentum.py`의 `LLMMomentumStrategy`)은 시간 기반 전략이라
  ...
  (실시간 시세 콜백, **종목별** 순손익 -설정값 이하 손절), 설정 주기마다 보유 종목 전체를 묻는
  AI 매도 판단, 15:15 강제청산이 담당한다. 15:35에는 일일 리포트 다음으로
...
- **그 자리를 AI 매도 판단이 대신한다** (`runtime.watch_ai_exit` → `src/llm/exit_advisor.py`).
  ...
  - **이익 반납 감시** (`src/core/exit_drawdown.py` → `TradingEngine._track_exit_drawdown`).
    **실시간 시세 콜백에서** 종목별 당일 순손익 고점을 따라가고, **고점이
    ...
    +4.46%에서 +1.41%까지 반납하고 본전으로 끝난 것이 계기다. **판정만 종목별이고 팔 때는 그대로
    전량이며, 여기서 팔지는 않는다** — 손절과 달리 AI를 부를 시점만 앞당긴다
    (PRD 5.5-B "이익 반납 감시").
...
- **판정 단위는 손절과 AI 판단이 서로 다르다** (2026-09-18에 갈림 — 그 전에는 둘 다 보유 목록
  전체 합산이었다). **손절은 종목별**이라 손절선에 닿은 종목만 판다. **AI 매도 판단은 여전히
  보유 종목 전량**이다 — "지금 전부 정리할지"를 묻는 별개 판단이라 종목별로 쪼개지 않는다.
  그래서 한 종목이 손절선(기본 -2%)에는 못 미치게 무너져도(예: -1%) 다른 종목이 버티면
  AI가 팔지 않는 한 15:15 강제청산까지 갈 수 있다 — 의도된 동작이다 (PRD 5.5-B).
