...
- **기본 상태는 손절 + 단순익절이다** (2026-08-13에 바꿈). 합산 퍼센트 익절('적용')만
  기본으로 꺼져 있다. 2026-08-12에 익절 두 방식을 모두 껐다가(실매매 27건을 당일 고가와
  대조해 보니 익절이 상방을 잘라 −209,464원 → 익절 없이 손절만이면 +59,320원, PRD 10절
  "익절 기본 해제") 단순익절만 되돌린 것이다 (PRD 10절 "단순익절 기본 적용"). 그 대조
  결과 자체는 철회되지 않았다 — 표본이 27건/12거래일로 작다는 전제 위의 재판단이다.
- 익절 방식은 둘 중 하나다. **"단순익절"만 종목별이다** — 켜면 익절선이
  ...
  - 세 체크박스 모두 `.env`에 저장하지 않는다. 기본 상태(**손절 + 단순익절**)는
