...
- 익절/손절 판정은 가격 변동률이 아니라 **순손익률**(수수료·세금·슬리피지를 뺀 값) 기준이다.
  `TAKE_PROFIT_PERCENT` / `STOP_LOSS_PERCENT`를 UI에서 조정하며, `.env.example`의 값과
  사용자의 실제 `.env` 값이 다를 수 있으니 동작을 따질 때는 **`.env`를 직접 확인한다**.
- **기본 상태는 손절 + 합산 퍼센트 익절이다** (2026-08-18에 바꿈). 단순익절만
  기본으로 꺼져 있다. 익절 기본값은 네 번 바뀌었다: 단순익절(도입) → 둘 다 해제
  (2026-08-12) → 단순익절(2026-08-13) → **퍼센트 익절(2026-08-18)**. 2026-08-12의
  근거(실매매 27건을 당일 고가와 대조해 익절이 상방을 잘라 −209,464원 → 익절 없이
  손절만이면 +59,320원, PRD 10절 "익절 기본 해제")는 **철회되지 않았다** — 표본이
  27건/12거래일로 작다는 전제 위의 재판단이며, 그 분석은 익절선 0%·0.5%·1%·2%·2.3%·
  3%·4% 어느 값도 "익절 없음"보다 못했다고 적고 있다. 지금 기본값은 그 결론과
  어긋나는 방향이므로, 실매매 데이터로 다시 대조해 볼 값이다.
- 익절 방식은 둘 중 하나다. **"단순익절"만 종목별이다** (기본 해제) — 켜면 익절선이
  `take_profit_ratio`가 아니라 **0**이 되고, **그 종목의** 순손익률이 0을 초과하면
  `check_simple_take_profits`가 그 종목만 골라내고 `_execute_simple_take_profit`이
  그 종목만 판다. 나머지는 계속 보유한다. 익절(%) 입력란은 그동안 비활성화된다.
  - 익절 "적용"(합산 퍼센트 익절)과 **배타적**이다 — 한쪽을 켜면 다른 쪽이 자동으로
    꺼지고, 둘 다 끄면 익절이 없다. 상위/하위 스위치가 아니다.
  - 엔진은 **합산 판정(`check_portfolio_exit`)을 먼저** 본다. 이익 종목이 먼저 빠지면
    남은 손실을 상쇄할 것이 없어져 합산 손절이 더 쉽게 걸린다 — 알고 택한 순서 의존성이다.
  - 세 체크박스 모두 `.env`에 저장하지 않는다. 기본 상태(**손절 + 합산 퍼센트 익절**)는
    **프로그램을 새로 시작할 때만** 적용되고, 켜 둔 동안 바꾼 상태는 종료할 때까지
    유지된다 — 매일 08:00에 되돌리던 UI 타이머는 2026-08-13에 제거했다.
