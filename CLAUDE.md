...
손절 기준, 매도 방식 같은 동작 규칙을 바꿀 때는 PRD를 먼저 갱신하고 코드를 맞춘다 —
...
- 1호 전략(`src/strategy/llm_momentum.py`의 `LLMMomentumStrategy`)은 시간 기반 전략이라
  ...
  AI 매도 판단(종목별 판정, 확정 2026-09-19), 15:15 강제청산이 담당한다. 15:35에는 일일 리포트 다음으로
  `DailyWorkflow.review_recommendations`가 그날
...
- **시스템 레벨**: `RiskManager`가 일일 손실 한도·총노출 비중을 전략과 무관하게 강제한다
  ...
  1호 전략에는 적용되지 않는다.
- 손절(`RiskManager.check_position_exits` → `TradingEngine._check_portfolio_exit`)은 키움 REST가
  스탑오더(조건부 예약주문)를 지원하지 않아, 이 프로그램이 떠 있는 동안의 실시간 시세 감시가
  **1차이자 사실상 유일한 하방 청산 수단**이다 — 앱이 꺼지거나 WebSocket이 끊기면 그 사이 손절도
  멈춘다.
...
- **익절은 없다** — 자동 청산은 2026-09-09에 걷어냈고, 설정 자체(`TAKE_PROFIT_PERCENT`)도
  2026-09-18에 완전히 지웠다. 단순익절(종목별 0% 익절)도 2026-09-09에 함께 사라졌다.
...
- **그 자리를 AI 매도 판단이 대신한다** (`runtime.watch_ai_exit` → `src/llm/exit_advisor.py`).
  ...
  `AI_EXIT_INTERVAL_MINUTES`)마다 보유 종목을 **하나씩** LLM에 보여 주고 **그 종목을 지금
  매도할지 종목마다** 묻는다(확정 2026-09-19 — 그 전에는 보유 종목 전체를 한 번에 정리할지
  묻는 단일 판정이었다). 응답에서 빠진 종목과 보유하지 않은 종목코드(환각 포함)는 **보유**로
  떨어진다. 판단 기준은 **당일 순수익(수수료·세금 차감)의 최대화**다(확정 2026-09-22 — 덜
  잃은 것은 잘한 것이 아니다). 순손익이 플러스인 종목은 이익을 제때 확정하고(고정 익절선이 없어
  이익 실현을 정하는 것은 이 판단뿐이다), 손실 중인 종목은 아침 시나리오가 무너졌다는 구체적
  근거가 있을 때 판다 — 현재가가 아침 전망의 하락 경로 가격을 깬 것도 근거이고, "손절선이
  관리한다"는 보유 근거가 아니다. AI는 매도 여부만 내고 **가격은 내지 않는다**
  — 실행은 매도로 판정된 종목만 `_execute_portfolio_exit` 경로의 시장가이며 `exit_reason`에
  `ai_judgment`로 남는다.
  - **손절이 항상 먼저다.** 실시간 시세 콜백에서 도므로 AI를 기다리지 않는다. AI는 손절선에
    닿기 전 구간에서만 청산을 앞당긴다.
  - **프롬프트에는 손절선·익절 기준선을 주지 않는다** (2026-09-18). 종전에는 손절선까지 남은
    거리와 익절 기준선(참고값)을 넘겼는데, 로그에서 AI가 "손절선까지 여유가 있다"를 주된 보유
    근거로 반복해서 쓰는 것이 확인돼 숫자 자체를 뺐다 — 지시("참고 지표일 뿐")보다 데이터가
    셌다. 손절이 이 판단과 무관하게 실시간으로 도는 것은 그대로다.
  ...
  - **이익 반납 감시** (`src/core/exit_drawdown.py` → `TradingEngine._track_exit_drawdown`).
    **실시간 시세 콜백에서** 종목별 당일 순손익 고점을 따라가고, **고점이
    `AI_EXIT_DRAWDOWN_PERCENT`(%p, 기본 1, `.env` 전용) 임계값 이상인 종목만** 고점 대비 그만큼
    반납하면 호출 주기를 기다리지 않고 AI 판단을 앞당긴다 — 임계값 미만인 고점에서 반납이 잡히면
    매수가 근처의 출렁임까지 "이익 반납"으로 잘못 세므로 2026-09-18에 이 최소 고점 요건을
    추가했다(관측된 발동 11건 중 8건이 고점 +1% 미만이었다). 고점은 15분 주기 사이를 스쳐
  ...
  - **장중 공시도 함께 본다** (`runtime.watch_disclosures` → `src/core/disclosure_watch.py`).
    ...
    AI 판단을 앞당기지만, **곧바로 팔지는 않는다** — 키워드 판정은 신호일 뿐, 그 종목을 팔지는
    AI가 내용을 보고 정한다. 판정이 종목별이라(2026-09-19) 한 종목의 공시가 다른 종목까지
    끌고 내려가지도 않는다 (PRD 5.5-B "장중 공시").
...
- **판정 단위는 손절도 AI 판단도 종목별이다** (손절은 2026-09-18에, AI 매도 판단은
  2026-09-19에 각각 종목별로 바뀌었다 — 그 전에는 둘 다 보유 목록 전체 합산/전량이었다).
  **손절**은 손절선에 닿은 종목만, **AI 매도 판단**은 매도로 정한 종목만 판다. **전량으로
  남은 것은 15:15 강제청산뿐이다.** 그래서 한 종목이 손절선(기본 -2%)에는 못 미치게
  무너져도(예: -1%) 다른 종목의 판정과 섞이지 않고, AI가 그 종목을 팔지 않는 한 15:15
  강제청산까지 갈 수 있다 — 의도된 동작이다 (PRD 5.5-B).
