...
- **익절은 기본으로 꺼져 있다** (2026-08-12에 바꿈). 기본 상태는 **손절만**이며, 평소
  청산은 손절 아니면 15:15 강제청산이다. 실매매 27건을 당일 고가와 대조해 보니 익절이
  상방을 잘라 순손익을 깎고 있었다 (PRD 10절 "익절 기본 해제" — 익절 없이 손절만 두면
  −209,464원 → +59,320원). 익절을 켜는 것은 그날 하루짜리 판단이다.
- 익절을 켤 때는 두 방식 중 하나다. **"단순익절"만 종목별이다** — 켜면 익절선이
  ...
  - 세 체크박스 모두 `.env`에 저장하지 않고, 매일 08:00(`EXIT_FLAG_RESET_TIME`)에 UI
    타이머가 기본 상태(**손절만**)로 되돌린다.
