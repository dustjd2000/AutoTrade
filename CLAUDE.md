...
- 익절/손절 판정은 가격 변동률이 아니라 **순손익률**(수수료·세금·슬리피지를 뺀 값) 기준이다.
  ...
  둘 다 UI에서 조정한다.
- **평소 익절은 그 설정값이 아니라 "단순익절"로 동작하며, 이것만 종목별이다**
  (2026-08-12에 추가). UI "단순익절적용" 체크박스가 기본 켜짐이고, 켜진 동안 익절선은
  `take_profit_ratio`가 아니라 **0**이다 — **그 종목의** 순손익률이 0을 초과하면
