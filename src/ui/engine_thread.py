...
class EngineThread(QThread):
    ...
    def set_exit_flags(self, ai_exit: bool, stop_loss: bool) -> bool:
        ...
        다른 리스크 설정과 달리 엔진 재시작 경로를 타지 않는다. 재시작 사이에는 WebSocket이
        끊겨 감시가 멈추는데, 정작 손절을 끄고 싶은 순간에 그 공백이 생긴다. 상태를 `.env`에
        저장하는 것(다음에 엔진이 새로 뜰 때 쓰인다)은 호출하는 쪽인 `MainWindow`가 맡는다.
