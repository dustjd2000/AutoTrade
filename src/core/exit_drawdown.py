...
# 날에는 그만큼 자주 발동한다. 공시 트리거(5회)보다 넉넉한 것은 임계치가 1%p로 낮아
# 발동 자체가 잦고, 정규 주기가 30분이면 트리거가 빠진 자리를 메울 것이 그만큼 적기
# 때문이다 — 상한이 낮으면 오전에 다 쓰고 오후에는 30분 주기만 남는다.
MAX_URGENT_TRIGGERS_PER_DAY = 10
