...
BUY_TIME = dt_time(9, 10)           # 자금 산정 → 목표 매수가 지정가 매수
CANCEL_UNFILLED_TIME = dt_time(9, 40)  # 미체결 매수 취소 → 매수 결과 메일 (매수 +30분)
...
def close_out(workflow: DailyWorkflow, engine: TradingEngine) -> Callable[[], None]:
    ...
    09:40 `cancel_unfilled_buys`가 정상적으로 돌았다면 여기서 거둘 주문은 없다. 이 호출은
    09:40을 놓친 경우를 위한 그물이다 — 엔진을 09:40 이후에 켜면 `TimeScheduler`가 지난
