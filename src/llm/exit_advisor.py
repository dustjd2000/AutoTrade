...
**"손절선이 관리한다", "손절선에 맡긴다"는 보유 근거가 되지 못합니다.** 시나리오가 깨진
손실 종목을 15:15 강제청산까지 들고 가면 손실이 확정될 뿐입니다.

구체적 근거는 공시만이 아닙니다. 아래 중 하나라도 해당하면 시나리오가 무너진 것입니다.
- 현재가가 **아침 전망이 적어 둔 하락 경로의 가격**(예: "돌파하지 못하면 X원 부근까지
  되밀릴 수 있다"의 X원)에 닿았거나 그 아래로 내려갔다 — 아침에 스스로 적은 실패 경로가
  현실이 된 것입니다.
...
2. **아침 전망의 유효성** — 이 종목을 고를 때 본 시나리오(`outlook`/`reason`)가
   ...
   것입니다.** 달성된 시나리오는 더 들고 있을 근거가 되지 못합니다 — 목표 매도가를
   넘어섰고 순손익률이 플러스라면 그것 자체가 매도 근거입니다.
