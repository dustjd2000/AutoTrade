...
- ~~다음 보유일에 로그를 확인한다~~ → **완료 (2026-09-01 14:12).** 진단 로그에 `매입수수료 0.0, 예상 매도비용 3.0`과 응답 키 전체가 찍혔다. `pur_cmsn`·`sell_cmsn`·`tax`가 모두 있어 순손익은 키움 실제값으로 계산된다. `_fee_fields`의 후보 키를 넓힐 필요가 없다.
- **부분체결 뒤 매도한 경우는 여전히 매수예정 표로 돌아온다.** 전량 체결만 기억하고 관찰한 수량까지는 기억하지 않는다 (`_observe_settled`, 2026-09-01). 흔한 경우가 아니라 한계로 남겼다.
- 화면 실물 확인은 2026-09-01 재시작 뒤 완료했다.
